...
const ItineraryDetailPage = () => {
  ...
  useEffect(() => {
    ...
    /**
     * Backend returns: { message: "...", data: <Itinerary> }
     * We extract .data to get the actual itinerary document.
     */
    getJson(`/api/itineraries/${itineraryId}`)
      .then((res) => {
        const doc = res?.data ?? res; // handle both { data: ... } and raw doc
        if (!doc || !doc.title) {
          setNotFound(true);
        } else {
          setItinerary(doc);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50 px-4">
        <div className="text-center">
          ...
          <button
            onClick={() => navigate("/")}
            className="px-6 py-2.5 bg-red-600 text-white font-semibold rounded-xl hover:bg-red-700 transition"
          >
            Go to Home
          </button>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="flex flex-wrap gap-3 mb-8">
          <button
            ...
            className="px-6 py-2.5 bg-red-600 hover:bg-red-700 text-white font-bold rounded-xl shadow-md shadow-red-200 transition-all text-sm"
          >
            Get a Quote
          </button>
          <Link to="/contactUs">
