 ...
 *
 * Shown when a user clicks "Know More" on a Trending destination card.
 *
 * Route:  /trending/:destinationId
 * Params: destinationId — URL-slug of the destination (e.g. "goa", "kerala")
 *
 * Fetches itineraries that:
 *   • belong to the selected destination
 *   • are classified as "Trending"
 *
 * API:  GET /api/itineraries?destination=<name>&classification=Trending
 */
...
/** Convert a URL slug back to a display title ("goa-beach" → "Goa Beach"). */
            ...
            onClick={() =>
              navigate(
                itinerary.type === "international"
                  ...
                  : `/domestic-itinerary/${destinationId}/${itinerary.slug}`
              )
            }
            className="flex-1 border border-red-600 text-red-600 hover:bg-red-600 hover:text-white text-sm font-semibold py-2 rounded-lg transition-colors"
          >
            View Details
          </button>
        </div>
...
const TrendingDestinationCards = () => {
  ...
  useEffect(() => {
    ...
    /**
     * Fetch itineraries for this destination that are tagged "Trending".
     * API: GET /api/itineraries?destination=<name>&classification=Trending
     */
    getJson(
      `/api/itineraries?destination=${encodeURIComponent(destinationId)}&classification=Trending`
    )
      .then((res) => {
        setItineraries(res?.data || []);
      })
      .catch((err) => {
        console.error("Failed to load trending itineraries:", err);
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      >
        ...
        <button
          onClick={() => navigate("/")}
          className="absolute top-5 left-5 flex items-center gap-1.5 text-red-200 hover:text-white text-sm font-semibold transition-colors"
        >
          ← Back
        </button>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-10">
        ...
        {!loading && !error && itineraries.length === 0 && (
          <div className="flex flex-col items-center justify-center py-24 text-center">
            ...
            <button
              onClick={() => navigate("/")}
              className="mt-6 px-6 py-2.5 bg-red-600 text-white text-sm font-semibold rounded-xl hover:bg-red-700 transition"
            >
              Back to Home
            </button>
          </div>
